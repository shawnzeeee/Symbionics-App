    ...
    <div class="flex flex-col items-center flex w-1/2">
      ...
      <div class="relative flex items-center justify-center mt-10">
        ...
        <div class="flex flex-col items-center">
          <span class="text-[#075776] text-base font-semibold drop-shadow mb-1"
            >Close</span
          >
          ...
          <span class="text-[#075776] text-base font-semibold drop-shadow mt-1"
            >Open</span
          >
        </div>
      ...
      <div
        class="absolute bottom-5 right-5 flex flex-col items-end space-y-3 z-10"
      >
        <button
          @click="saveSettings()"
...
import { trainSVM, beginPylslStreamNoFileWrite, loadFileToGlove, endMusePylslStream, updateCSV, fetchSensitivityValues, eject_usb} from "../api.js";
...
onMounted(async () => {
  ...
  try {
    const fetch_sensitivity_response = await fetchSensitivityValues(
      selectedFile.value
    );
    if (fetch_sensitivity_response) {
      attention_adder.value = fetch_sensitivity_response.data.attention_adder;
      attention_subtractor.value =
        fetch_sensitivity_response.data.attention_subtractor;
    }
  } catch (error) {
    console.log(error);
  }
  try {
    ...
    const stream_response = await beginPylslStreamNoFileWrite(
      selectedFile.value
    );
